...
bool load_binary_data(const std::string &filename, uint16_t *data)
{
...
// Save binary data
bool save_binary_data(const std::string &filename, uint16_t *data)
{
...
void print_data_stats(uint16_t *data, const std::string &name)
{
    ...
    std::cout << name << " statistics: min=" << min_val << ", max=" << max_val
              << ", mean=" << sum / DATA_SIZE << std::endl;
}

// Compare results
int compare_results(uint16_t *result, uint16_t *golden, uint16_t *in0, uint16_t *in1,
                    uint32_t config)
{
    int errors = 0;
    uint16_t max_diff = 0;
    ...
    for (int i = 0; i < DATA_SIZE; ++i) {
        if (result[i] != golden[i]) {
            diff = std::abs(result[i] - golden[i]);
            if (diff > max_diff) {
                max_diff = diff;
            }
            errors++;
...
bool run_test(int config, uint16_t *in0, uint16_t *in1, uint16_t *out, uint16_t *golden_data_ptr,
              const std::string &data_path, uint16_t *mask_data)
{
    std::cout << "\n--- Testing Config " << config << " ---" << std::endl;

    // Select input1 based on config (only config 2 uses mask)
    uint16_t *current_in1 = (config == 2) ? mask_data : in1;

    // Stage 1: Compute
    uint32_t current_stage = STAGE_COMPUTE;
    ...
    std::string output_filename =
        data_path + "hls_output_config_" + std::to_string(config) + ".bin";
...
{
    // Allocate memory
    uint16_t *in0 = new uint16_t[DATA_SIZE];
    uint16_t *in1 = new uint16_t[DATA_SIZE];
    uint16_t *out = new uint16_t[DATA_SIZE];
    uint16_t *mask = new uint16_t[DATA_SIZE];
    uint16_t *golden_data[7];
    for (int i = 0; i < 7; ++i) golden_data[i] = new uint16_t[DATA_SIZE];
    ...
    for (int i = 0; i < 7; ++i) {
        std::string golden_file =
            data_path + "golden_out_config_" + std::to_string(i) + "_bf16.bin";
    ...
    for (int config = 0; config < 7; ++config) {
        ...
        stage = STAGE_STORE;
        activation_accelerator(in0, in1, out, stage, config);
        std::cout << "Output results saved to: " << data_path << "hls_output_config_" << config
                  << ".bin" << std::endl;
        ...
        if (errors == 0) {
            ...
            std::cout << "Config " << config << " test failed with " << errors << " errors"
                      << std::endl;
        }
    }
