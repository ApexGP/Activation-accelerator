...
void FPSub::set_input(uint32_t X, uint32_t Y)
{
...
void FPSub::IntDualSub_17(uint32_t X, uint32_t Y, uint32_t &XmY, uint32_t &YmX)
{
...
void FPSub::Normalizer_Z_16(uint16_t X, uint8_t &Count, uint16_t &R)
{
    uint16_t level5 = X;
...
void FPSub::RightShifterSticky15(uint16_t X, uint8_t S, uint32_t &R, bool &Sticky)
{
    uint8_t ps = S & 0x1F;
...
void FPSub::IntAdder_18(uint32_t X, uint32_t Y, bool Cin, uint32_t &R)
{
    R = (X + Y + (Cin ? 1 : 0)) & 0x3FFFF;  // 18-bit
}

// 24-bit integer adder
void FPSub::IntAdder_24(uint32_t X, uint32_t Y, bool Cin, uint32_t &R)
{
    R = (X + Y + (Cin ? 1 : 0)) & 0xFFFFFF;  // 24-bit
}

void FPSub::stage_input_and_swap()
{
    uint32_t inX = X_reg;
    uint32_t inY = Y_reg;
    ...
    // FPSub core: flip Y's sign bit (line 408: mY = {inY[24:23], ~(inY[22]),inY[21:0]})
    uint32_t mY = ((inY & 0x1800000) | ((~inY & 0x400000) | (inY & 0x3FFFFF))) & 0x1FFFFFF;

    // Swap X and mY
    uint32_t newX = swap ? mY : inX;
    uint32_t newY = swap ? inX : mY;
...
void FPSub::stage_close_far_path()
{
...
void FPSub::clock_step()
{
    /*
     * FPSub computes X - Y (by flipping Y's sign bit)
     */
